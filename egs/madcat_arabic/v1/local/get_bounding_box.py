...
def set_line_image_data(bounding_box, line_id, image_file_name, image_fh):
...
def get_line_images_from_page_image(image_file_name, madcat_file_path, mydata):
    ...
    for node in zone:
        ...
        val.append(bounding_box.corner_points)
        mydata[line_image_file_name] = val
...
def main():
    ...
    data = "/Users/ashisharora/google_Drive/madcat_arabic/"
    args.database_path1 = download_dir1
    args.database_path2 = download_dir2
    args.database_path3 = download_dir3

    writing_conditions1 = os.path.join(download_dir1, 'writing_conditions.tab')
    writing_conditions2 = os.path.join(download_dir2, 'writing_conditions.tab')
    writing_conditions3 = os.path.join(download_dir3, 'writing_conditions.tab')
    ...
    image_file = os.path.join(output_directory, 'boundingbox.pickle')
    image_fh = open(image_file, 'wb')

    splits_handle = open(dataset_file, 'r')
    ...
    for line in splits_data:
        ...
        if prev_base_name != base_name:
            ...
            if madcat_file_path is not None:
                get_line_images_from_page_image(image_file_path, madcat_file_path, my_data)
    pickle.dump(my_data, image_fh, protocol=pickle.HIGHEST_PROTOCOL)
